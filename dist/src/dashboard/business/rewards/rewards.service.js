...
let RewardsService = class RewardsService {
    ...
    async getRewardStats(userId, eventId) {
    ...
    async purchaseBundle(userId, eventId, dto) {
        const booth = await this.getBoothForUser(userId);
    ...
    async allocateTickets(userId, eventId, dto) {
        const booth = await this.getBoothForUser(userId);
        return await this.prisma.$transaction(async (tx) => {
            ...
            if (!balance || balance.balance < dto.emails.length) {
                throw new common_1.BadRequestException(`Insufficient bulk ticket balance. Available: ${balance?.balance || 0}`);
            }
            ...
            const allocations = await Promise.all(dto.emails.map(email => tx.rewardAllocation.create({
                data: {
                    email,
                    boothId: booth.id,
                    eventId,
    ...
    async getRewardCampaigns(userId, eventId) {
        const booth = await this.getBoothForUser(userId);
    ...
    async getIntegrationStatus(userId, eventId) {
        const booth = await this.getBoothForUser(userId);
    ...
    async listIntegrations(userId, eventId) {
        const booth = await this.getBoothForUser(userId);
        const integrations = await this.prisma.loyaltyIntegration.findMany({
            where: { boothId: booth.id, eventId }
        });
    ...
    async getRewardLogs(userId, eventId, search) {
        const booth = await this.getBoothForUser(userId);
    ...
    async getAbuseAlerts(userId, eventId) {
        const booth = await this.getBoothForUser(userId);
    ...
    async blockIP(userId, ip, reason) {
        const booth = await this.getBoothForUser(userId);
